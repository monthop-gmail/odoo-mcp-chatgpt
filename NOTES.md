...
### `odoo_read_group` บอกเมื่อผลถูกตัด

พฤติกรรมนี้ตามหลัง cf-odoo-mcp-server ซึ่งเจอบั๊กจากการใช้งานจริงบน Claude chat
ว่าการใส่ `limit` ทำให้ผลถูกตัดโดยไม่มีสัญญาณ agent จึงบวกยอดจากสิ่งที่มองเห็น
แล้วได้ตัวเลขต่ำกว่าความจริง

ตอนนี้ถ้าถูกตัดจะได้ `has_more` กับ `total_records` มาด้วย และ `groups` เป็น key
เสมอไม่ว่าจะถูกตัดหรือไม่ ทดสอบกับ Odoo 19 CE แล้ว — `limit: 3` บนกลุ่มที่มี
5 กลุ่มได้ `has_more: true` กับ `total_records: 41` ส่วนไม่ใส่ limit ได้ครบ 5 กลุ่ม
โดยไม่มี `has_more`

แก้ทั้งสอง repo พร้อมกันเพราะเอกสารสัญญาไว้ว่าพฤติกรรม tool เหมือนกัน ถ้าแก้
ที่เดียวคำสัญญานั้นก็เป็นเท็จ

### ฝั่งเขียนจาก ChatGPT ผ่านท่อ

สั่งสร้าง contact ชื่อไทย พร้อมจังหวัด และแอบใส่ `complete_name` ที่เขียนไม่ได้เข้าไป

| | ผล |
| --- | --- |
| สร้าง record | ✅ id 50 · partner 41 → 42 |
| ชื่อไทย | ✅ ไม่เพี้ยน |
| `complete_name` | ✅ รายงานว่าไม่เข้า พร้อมอธิบายว่าเป็น computed field |
| `city` | ❌ **ว่างเปล่า ทั้งที่รายงานว่าสำเร็จ** |

`fields_not_applied` ตอบคำถามที่ค้างมาตั้งแต่ทำ feature นี้ — agent รายงานตามจริง
ไม่ได้ปัดเศษว่าสำเร็จหมด

### ความล้มเหลวชั้นที่ tool มองไม่เห็น

`city` ที่หายไปไม่ใช่บั๊กของ tool ทดสอบซ้ำโดยส่ง `city` เองแล้วเก็บได้ปกติ และ
`fields_not_applied` ก็รายงานแค่ `complete_name` ซึ่งถูกต้อง เพราะ `city` ไม่เคย
ถูกส่งมาให้เทียบ

สิ่งที่เกิดขึ้นคือ agent ตีความ "สุราษฎร์ธานี" เป็นประเทศไทยแล้วใส่ `country_id`
แทน `city` ไม่ผิดในแง่ข้อมูล แต่ไม่ตรงกับที่สั่ง แล้วรายงานว่าสำเร็จ

| ชั้น | อาการ | อะไรจับได้ |
| --- | --- | --- |
| Odoo ทิ้งค่า | ส่งไปแล้วไม่เข้า | `fields_not_applied` |
| ดึงมาไม่ครบ | นับผิด | `has_more` |
| **agent ไม่ส่งไปเลย** | **สั่งอย่าง ทำอีกอย่าง** | **ไม่มี** |

ชั้นที่สามอยู่นอกขอบเขตของ MCP server โดยสิ้นเชิง — server เห็นแค่สิ่งที่ agent ส่งมา
ไม่เห็นสิ่งที่ผู้ใช้สั่ง ทางเดียวที่จับได้คือผู้ใช้อ่าน `record` ที่คืนมา ซึ่งมี
`city: false` อยู่ในนั้น แต่ agent ไม่ได้เอ่ยถึงเพราะไม่รู้ว่าตัวเองพลาด

**ข้อควรระวังตอนวัดผล tool**: ถ้าจะทดสอบว่า tool ทำงานถูกไหม ต้องดู `record` ที่
คืนมาทุกครั้ง ไม่ใช่เชื่อสรุปของ agent — ไม่งั้นจะแยกไม่ออกว่าที่ผิดคือ tool หรือ
คือ agent ที่ส่งมาไม่ครบตั้งแต่แรก

### เรื่องโควตาข้อความ### เรื่องโควตาข้อความ
